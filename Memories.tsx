...
export default function Memories({ members: _members, knotId }: { members: any[], knotId?: string }) {
  ...
  async function loadMemories() {
    ...
    if (photosFetchError) setLoadError('Could not load photos. Try refreshing.')
  ...
  async function loadComments(photoId: string) {
    ...
    if (error) { setCommentError('Could not load comments.'); return }
    setComments(data || [])
  }
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (uploaded > 0) {
      ...
      if (postError) console.error('Memories feed post error:', JSON.stringify(postError))
    }

    if (failed > 0) {
      setUploadError(uploaded > 0
        ? `${uploaded} photo${uploaded > 1 ? 's' : ''} uploaded, ${failed} failed. Try the failed ones again.`
        : `Could not upload ${failed > 1 ? 'those photos' : 'that photo'}. Please try again.`)
    }
  ...
  async function deletePhoto(photo: any) {
    ...
    if (dbError) { setDeleteError('Photo file removed, but the record failed to delete.'); return }
    setViewPhoto(null)
    await loadMemories()
  }
