...
async function main() {
  ...
  // 1. In the upload handler, set coverSignedUrl immediately after upload
  //    so the image updates without waiting for useEffect
  src = src.replace(
    `const updated = { ...activeKnot, cover_url: publicCoverUrl }; setActiveKnot(updated); setKnots(ks => ks.map(k => k.id === activeKnot.id ? updated : k))`,
    `const updated = { ...activeKnot, cover_url: publicCoverUrl }; setActiveKnot(updated); setKnots(ks => ks.map(k => k.id === activeKnot.id ? updated : k)); setCoverSignedUrl(publicCoverUrl)`
  )

  // 2. Keep cache-bust on upload URL
  src = src.replace(
  ...
  // 3. useEffect: also add cache-bust for when switching knots
  src = src.replace(
