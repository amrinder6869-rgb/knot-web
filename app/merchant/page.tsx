    ...
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 40 }}>
        <svg width="22" height="22" viewBox="0 0 44 44" fill="none">
          <circle cx="17" cy="17" r="10" style={{ stroke: 'var(--yellow)' }} strokeWidth="3" fill="none"/>
          <circle cx="27" cy="27" r="10" style={{ stroke: 'var(--yellow)' }} strokeWidth="3" fill="none" opacity="0.5"/>
        </svg>
        <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text)' }}>
          kn<span style={{ color: 'var(--yellow)' }}>o</span>t <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text3)' }}>for business</span>
        </span>
      </div>

      <h1 style={{ fontSize: 32, fontWeight: 800, color: 'var(--text)', marginBottom: 16, letterSpacing: '-0.5px', lineHeight: 1.2 }}>
        Fill your tables with confirmed groups
      </h1>
      <p style={{ fontSize: 16, color: 'var(--text2)', lineHeight: 1.6, marginBottom: 40 }}>
      ...
      <button onClick={() => { setAuthMode('signup'); setStep('auth'); setError('') }}
        style={{ width: '100%', padding: '14px', background: 'var(--yellow)', border: 'none', borderRadius: 10, color: 'var(--text)', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 12 }}>
        List my restaurant
      </button>
      <button onClick={() => { setAuthMode('signin'); setStep('auth'); setError('') }}
        style={{ width: '100%', padding: '12px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text2)', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
  ...
  if (step === 'auth') return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '80px 24px' }}>
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', marginBottom: 8 }}>{authMode === 'signin' ? 'Sign in to your account' : 'Create your account'}</h2>
      ...
      <button onClick={authMode === 'signin' ? signIn : signUp} disabled={loading}
        style={{ width: '100%', padding: '12px', background: 'var(--yellow)', border: 'none', borderRadius: 8, color: 'var(--text)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 10, opacity: loading ? 0.6 : 1 }}>
        {loading
          ? (authMode === 'signin' ? 'Signing in...' : 'Creating account...')
          : (authMode === 'signin' ? 'Sign in' : 'Create account')}
      </button>
      <button onClick={() => { setAuthMode(authMode === 'signin' ? 'signup' : 'signin'); setError('') }} disabled={loading}
        style={{ width: '100%', padding: '12px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text2)', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', opacity: loading ? 0.6 : 1 }}>
        {authMode === 'signin' ? 'Need an account? Create one' : 'Sign in instead'}
      </button>
      <button onClick={() => setStep('intro')}
        style={{ width: '100%', padding: '10px', background: 'transparent', border: 'none', color: 'var(--text3)', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginTop: 8 }}>
        Back
      </button>
    </div>
  )
