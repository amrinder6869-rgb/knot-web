...
export default function HomeFeed({ knots, onSelectKnot }: { knots: any[], onSelectKnot: (knot: any) => void }) {
  ...
  async function loadFeed() {
    ...
    // Group photos uploaded together
    ...
    // Filter out photo-related system posts (deduplicate)
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13 }}>
                        ...
                        <span style={{ color: 'var(--text2)', marginLeft: 6 }}>
                          added {item.photos.length} photo{item.photos.length > 1 ? 's' : ''}
                        </span>
                      </div>
                      {item.caption && (
                        <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 3, fontStyle: 'italic' }}>"{item.caption}"</div>
                      )}
                    </div>
