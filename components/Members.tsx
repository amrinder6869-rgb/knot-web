    ...
    <div style={{ maxWidth:720 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <div>
            ...
            {!submitted ? (
              <>
                <div style={{ display:'flex', gap:8, marginBottom:10 }}>
                  <button onClick={() => castVote('yes')} style={{ flex:1, padding:'8px', borderRadius:8, border:`1px solid ${myVote==='yes' ? 'var(--sage)' : 'rgba(76,175,135,0.3)'}`, background: myVote==='yes' ? 'var(--sage)' : 'var(--sage-soft)', color: myVote==='yes' ? '#fff' : 'var(--sage)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                    👍 Vote Yes
                  </button>
                  <button onClick={() => castVote('no')} style={{ flex:1, padding:'8px', borderRadius:8, border:`1px solid ${myVote==='no' ? 'var(--coral)' : 'rgba(232,98,74,0.3)'}`, background: myVote==='no' ? 'var(--coral)' : 'var(--coral-soft)', color: myVote==='no' ? '#fff' : 'var(--coral)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                    👎 Vote No
                  </button>
                  <button style={{ padding:'8px 12px', borderRadius:8, border:'1px solid var(--border2)', background:'var(--bg3)', color:'var(--text2)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>Abstain</button>
                </div>
                ...
                {myVote && (
                  <button onClick={submitVote} style={{ width:'100%', padding:'9px', borderRadius:8, border:'none', background:'var(--indigo)', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                    Submit vote
                  </button>
                )}
              </>
          ...
          {/* Splinter */}
          {showSplinter && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--indigo)', borderRadius:12, padding:16 }}>
              <div style={{ fontSize:13, fontWeight:700, color:'var(--indigo)', marginBottom:6, display:'flex', alignItems:'center', gap:6 }}>⑂ Start a new Knot?</div>
              <div style={{ fontSize:12, color:'var(--text2)', lineHeight:1.6, marginBottom:12 }}>
                Jamie didn't pass this time. You voted Yes — want to start a new Knot with the other Yes voters + Jamie? The original Brampton Crew continues unchanged.
              </div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={() => { setShowSplinter(false); alert('New Knot created with Priya + Jamie! 🎉') }} style={{ flex:1, padding:'8px', background:'var(--indigo)', border:'none', borderRadius:8, color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                  ⑂ Start new Knot
                </button>
                <button onClick={() => setShowSplinter(false)} style={{ padding:'8px 14px', background:'var(--bg3)', border:'1px solid var(--border2)', borderRadius:8, color:'var(--text2)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>Not now</button>
              </div>
            </div>
          )}

          <div style={{ marginTop:14, fontSize:12, color:'var(--text3)', lineHeight:1.8, padding:'10px 12px', background:'var(--bg2)', borderRadius:8, border:'1px solid var(--border)' }}>
            ℹ️ More Yes than No = accepted. Votes always anonymous. Jamie sees: "This Knot isn't open right now." 60-day cooldown if rejected.
          </div>
        </div>
      </div>
    </div>
