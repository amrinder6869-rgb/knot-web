...
// ─── 1. Create PostHangoutLoop component ─────────────────────────────────────

const loopContent = `'use client'
...
console.log('Created: components/PostHangoutLoop.tsx');

// ─── 2. Patch HangoutCard to import and use PostHangoutLoop ──────────────────

const cardPath = path.join(BASE, 'components\\HangoutCard.tsx');
let cardContent = fs.readFileSync(cardPath, 'utf8');

// Add import
const oldImport = `import { CrewSection } from '@/components/CrewSection'`;
const newImport = `import { CrewSection } from '@/components/CrewSection'
import { PostHangoutLoop } from '@/components/PostHangoutLoop'`;

if (cardContent.includes(oldImport)) {
  cardContent = cardContent.replace(oldImport, newImport);
  console.log('Import added to HangoutCard.tsx');
} else {
  console.log('SKIP: import already exists or pattern not found');
}

// Insert PostHangoutLoop just before the bill section
// It renders only when isDone and not cancelled
const oldBillSection = `      {(bills.length > 0 || (showBill && !isCancelled)) && (`;
const newBillSection = `      {isDone && !isCancelled && (
        <PostHangoutLoop
          hangout={hangout}
          knotId={knotId}
...
if (cardContent.includes(oldBillSection)) {
  cardContent = cardContent.replace(oldBillSection, newBillSection);
  fs.writeFileSync(cardPath, cardContent, 'utf8');
  console.log('PostHangoutLoop inserted into HangoutCard.tsx');
} else {
  console.log('ERROR: Could not find bill section anchor in HangoutCard.tsx');
}

console.log('\nSprint 1C complete.');
console.log('Post-hangout rating, photo prompt, and behavioral signal capture are live.');
